...
static void read_handler(void * args) 
{
    ...
    protocol->on_message(protocol, client, buffer_reader, msglen);
...
static void accept_conn_handler(void * args) 
{
    ...
    //Cria a bagagem do cliente com o mesmo número de protocolos, para carregar informações de cada protocolo
    client->bag = malloc(sizeof(void*) * server->protocol_stack->size);
    ...
    protocol_t * protocol = server->protocol_stack->bottom;
...
void set_client_bag(protocol_t * protocol, client_t * client, void* bag) {
    client->bag[protocol->id] = bag;
}

void * get_client_bag(protocol_t * protocol, client_t * client) {
    return client->bag[protocol->id];
} 
